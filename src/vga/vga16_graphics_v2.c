...
// #define TXCOUNT 38400 // Total pixels/8 (since we have 8 pixels per byte)
...
void initVGA() {
    ...
    clear_chan = dma_claim_unused_channel(true);
...
void clear() {
  dma_channel_set_read_addr(clear_chan, &zero, false);
  dma_channel_set_write_addr(clear_chan, vga_data_array, false);
  dma_channel_set_transfer_count(clear_chan, TXCOUNT, false);
  dma_start_channel_mask(1u << clear_chan);
}

int is_still_clearing() {
  return dma_channel_is_busy(clear_chan);
}
...
void drawPixel(short x, short y, char color) {
    ...
    // vga_data_array[bit] = (vga_data_array[bit] & ~(1u << bitIndex)) | (color << bitIndex);
...
void drawBoid(short x0, short y0, char color) {
/* Draw a circle outline with center (x0,y0) and radius r, with given color
 * Parameters:
 *      x0: x-coordinate of center of circle. The top-left of the screen
 *          has x-coordinate 0 and increases to the right
