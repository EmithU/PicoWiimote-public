...
// ACL payload size: Pico W examples often use small payloads for HID.
// 69 = 1 (HIDP header) + 4 (L2CAP) + 64 (report), but you can keep 1021 if you like.
#define HCI_ACL_PAYLOAD_SIZE        (169)   // or (69) or (1021), keep <= controller MTU
...
// SDP / RFCOMM / other profiles (all 0 if unused)
...
// SDP service records (HID host doesn’t need many, but some profiles use this)
...
// If you ever use GATT/ATT, this is the fixed ATT DB size
...
// Optional: stdin command interface for some examples
